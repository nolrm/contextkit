...
- **Placeholder file** (only contains "Content will be generated by running: ck analyze" or `[YOUR_DOMAIN]` markers) → **Replace** the entire content with generated content.
...
This ensures that standards files evolve and improve over multiple `ck analyze` runs without losing prior work.
