...
class UpdateCommand {
  ...
  // Downloads a file but logs a warning and continues if it fails.
  // Use for non-critical files (types, scripts) that shouldn't abort the update.
  ...
  async downloadFiles(projectType, config = {}) {
    ...
    try {
      ...
      // Download types (optional — TypeScript-only; failures do not abort the update)
      await this._downloadOptional(
      ...
      // Download scripts (optional — failures do not abort the update)
      await this._downloadOptional(
      ...
      if (await fs.pathExists('.contextkit/types/type-check.sh')) {
        await fs.chmod('.contextkit/types/type-check.sh', '755');
      }
      if (await fs.pathExists('.contextkit/scripts/update.sh')) {
