...
Governed by \`.contextkit/config.yml\` → \`response_style\` (two independent flags — check both, they're not linked):

- \`chat_minimal_words\`: true → chat explanations are plain text, minimal words — state results/decisions directly, don't narrate step-by-step reasoning. false → respond normally.
- \`diagrams_in_docs\`: false → no Mermaid diagrams in generated \`.md\` files (see \`/analyze\` Output Density); prefer inline arrow chains for simple relationships, e.g. \`input → transform → output\`. true → diagrams allowed where they're the clearest option.

Flip either flag per-project. If \`config.yml\` is missing or the block isn't present, default to both rules on (terse chat, no diagrams).

## README.md Context Discovery

\`README.md\` is the standard context file at all 3 documentation levels. Before starting substantial work in a directory — a new feature, a component you'll modify non-trivially, or exploring an unfamiliar area — check for it:
...
`,
  ...
  async downloadFiles(projectType, options = {}) {
    ...
    try {
      ...
      await this.downloadManager.downloadFile(
        `${this.repoUrl}/commands/dev/compress.md`,
        '.contextkit/commands/dev/compress.md'
      );
      ...
      await this.downloadManager.downloadFile(
        `${this.repoUrl}/commands/agents/product-context.md`,
        '.contextkit/commands/agents/product-context.md'
      );
...
`;
  ...
  async createConfiguration(projectType, hookChoices, squadCi = false) {
    ...
    const config = {
      ...
      response_style: {
        chat_minimal_words: true,
        diagrams_in_docs: false,
      },
      paths: {
...
response_style:
  chat_minimal_words: ${config.response_style.chat_minimal_words}  # plain-text, terse chat explanations
  diagrams_in_docs: ${config.response_style.diagrams_in_docs}   # allow Mermaid in generated .contextkit/standards/*.md
...
# Standards content updates ck update couldn't auto-apply (local edits found) —
# see standards-migrations.md. The standards-aware skill reconciles these.
