...
This project uses ContextKit. Code style and architecture standards are auto-loaded via CLAUDE.md imports; product mission is too.

Loaded on demand via skills instead: testing conventions (\`test\` skill), dev workflows (\`agent-push-checklist\` skill), AI behavior rules and project terminology (\`standards-aware\` skill), architecture decisions and roadmap (\`product-context\` skill).
`;
...
Follow the testing standards loaded via the \`test\` skill (from testing.md).
...
Reads a product overview file, breaks it into logical scopes, and runs a multi-round pipeline for each scope: CTO briefs → 4 domain experts in parallel → CTO challenges → domain revisions → CTO authors final SPEC.md. Produces a structured \`.contextkit/spec/\` folder. Run once per scope — \`/spec\` picks up where you left off.
`
    );
...
`
    ...
    await this.writeGeneratedFile(
      '.claude/skills/compress/SKILL.md',
      `---
description: Measure a file's size, rewrite it for information density, and report the before/after diff
argument-hint: "<file or path to compress>"
allowed-tools: Read, Edit, Write, Glob, Grep, Bash
effort: normal
---

Read \`.contextkit/commands/dev/compress.md\` and execute the compression workflow.

Measure the target file's size, rewrite it for density while preserving every rule/behavior, then report the before/after line and word counts.
`
    );
...
`
    ...
    await this.writeGeneratedFile(
      '.claude/skills/product-context/SKILL.md',
      `---
description: Load architecture decisions and roadmap context before proposing changes
allowed-tools: Read, Glob, Grep
effort: low
---

Read \`.contextkit/commands/agents/product-context.md\` and execute the product-context workflow.

Load architecture decisions and roadmap context before proposing changes that might conflict with a prior decision or near-term plan.
`
    );
  }
  ...
  showUsage() {
    ...
    console.log(chalk.dim('    /compress     — Rewrite a file for density and report the diff'));
    console.log(chalk.dim('    /product-context — Load architecture decisions and roadmap'));
  }
}
